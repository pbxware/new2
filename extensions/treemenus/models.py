...
class MenuItem(models.Model):
    ...
    target = models.CharField(choices=TARGET_CHOICE, default='_self', max_length=10)
    selected_patterns = models.CharField(max_length=250, blank=True)
    ...
    def children_desc(self):
        _children = MenuItem.objects.filter(parent=self).order_by('-rank',)
        for child in _children:
            child.parent = self  # Hack to avoid unnecessary DB queries further down the track.
        return _children

    def children_active_desc(self):
        _children = MenuItem.objects.filter(parent=self, active=True).order_by('-rank',)
        for child in _children:
            child.parent = self  # Hack to avoid unnecessary DB queries further down the track.
        return _children
