...
@register.filter
def item_is_select(item, path):
    pattern = item.selected_patterns
    if pattern.strip() and re.compile(pattern).match(path):
        return True
    return False
...
def get_treemenus_static_prefix():
    if django.VERSION >= (1, 3):
        ...
        return PrefixNode.handle_simple("STATIC_URL") + 'treemenus'
    else:
        from django.contrib.admin.templatetags.adminmedia import admin_media_prefix
        return admin_media_prefix() + 'treemenus'
...
def show_menu(context, menu_name, menu_type=None):
    ...
    return context
register.inclusion_tag('%s/menu.jade' % APP_LABEL, takes_context=True)(show_menu)
...
def show_menu_item(context, menu_item):
    ...
    return context
register.inclusion_tag('%s/menu_item.jade' % APP_LABEL, takes_context=True)(show_menu_item)
